"""
Live Clustering — Traffic Segment Mapper (Phase 6)

Maps real external traffic-provider segments (TomTom/HERE) to nearest
RouteFlow road edges using cKDTree spatial node lookup and point-to-segment distance.

Features:
  - TRAFFIC_EDGE_MATCH_DISTANCE_METERS threshold (default 50m)
  - Point-to-line-segment projection distance
  - Match confidence calculation (HIGH <= 15m, MEDIUM <= 35m, LOW <= 50m)
  - Summary metrics: total_segments, matched_count, unmatched_count, match_percentage
  - Unmatched fallback visualization tags
  - Integration with Bounded TrafficMappingCache
  - READ-ONLY access to graph_service — ZERO graph/weight/routing modification
"""
import os
import math
import logging
...
from .traffic_mapping_cache import mapping_cache

logger = logging.getLogger("routeflow.live_clustering.traffic_segment_mapper")

# Configurable match distance threshold in meters
TRAFFIC_EDGE_MATCH_DISTANCE_METERS: float = float(
    os.environ.get("TRAFFIC_EDGE_MATCH_DISTANCE_METERS", 50.0)
)
...
def _point_to_line_segment_distance_m(
    ...
    """
    Computes exact distance (in meters) from point P(p_lat, p_lon)
    to line segment AB endpoints A(a_lat, a_lon) and B(b_lat, b_lon).
    Uses planar equirectangular approximation for fast local calculation.
    """
    # Convert lat/lon to local meters relative to P
    ...
    if ab_sq == 0.0:
        # Segment length is 0 (A and B are identical point)
        return math.hypot(ax, ay)

    # Project P onto line AB: t = dot(PA, AB) / |AB|^2
    apx = px - ax
    apy = py - ay
...
def map_segment(segment: Dict[str, Any]) -> Dict[str, Any]:
    """
    Maps a single normalized traffic segment to the nearest RouteFlow road edge.
    Returns structured mapping dict.
    """
    ...
    # 3. Fast spatial candidate search using cKDTree
    # Search radius 0.20 km (200m) to gather nearby nodes
    ...
    # 4. Evaluate incident edges for best point-to-segment match
...
def map_traffic_payload(traffic_payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    Processes a normalized traffic response dict from Phase 5,
    maps all segments to RouteFlow road edges, and attaches mapping summary metrics.
    """
