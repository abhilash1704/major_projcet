 ...
 *
 * Communicates product story:
 * LIVE VEHICLE CLUSTERING — Real-time vehicle estimation from location observations
 * DATA SOURCE: SIMULATION
 */
import { useState, useCallback, useRef } from "react";
import { MapPin, Search, Play, ChevronDown, Clock, Activity, Database, Info } from "lucide-react";
...
export const TopAnalysisBar = () => {
  ...
  const areaName = selectedArea?.name || "Silk Board Junction";

  return (
    <div className="w-full bg-white dark:bg-surface-dark border-b border-slate-200 dark:border-slate-800 flex flex-col shadow-xs font-sans">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                <Database className="w-3 h-3 text-slate-400" />
                DATA SOURCE: SIMULATION
              </span>
            </div>
            <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-0.5">
              Real-time vehicle estimation from location observations
            </p>
          </div>
        </div>

        {/* Small explanation directly below header */}
        <div className="mt-1.5 flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400 font-medium">
          <Info className="w-3.5 h-3.5 text-blue-500 shrink-0" />
          <span>
            Multiple nearby users can belong to the same vehicle. RouteFlow groups them to estimate the actual number of vehicles.
          </span>
        </div>
      </div>

      {/* ── 2. CONTROL BAR ROW ────────────────────────────────────────── */}
      <div className="px-5 py-2.5 bg-slate-50/70 dark:bg-slate-900/40 border-b border-slate-100 dark:border-slate-800/60">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3.5">
          ...
          <div className="shrink-0 pt-4 lg:pt-5">
            <button
              ...
              disabled={isLoading}
              className="w-full lg:w-auto px-4 py-2 rounded-lg font-bold text-xs bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/20 active:scale-95 transition flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
            >
      ...
      {/* ── 3. LIVE COMPACT STATUS STRIP ────────────────────────────────── */}
      <div className="px-5 py-2 bg-slate-900 text-slate-100 flex items-center justify-between flex-wrap gap-3 text-xs">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 font-black tracking-wide text-emerald-400 uppercase text-[11px]">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span>ANALYZING {areaName.toUpperCase()}</span>
          </div>

          <div className="hidden sm:flex items-center gap-1 text-slate-300 font-medium">
            <span className="text-slate-500">Radius:</span>
            <span className="font-bold text-white">{radiusFormatted}</span>
          </div>
        </div>
